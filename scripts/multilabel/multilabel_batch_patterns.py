...
def produce_patterns(video_name, params_list, pick_intervals):
  ...
  video_path = '{}/multi-raw/{}.txt'.format(file_storage_path, video_name)
  ...
  for params in params_list:
    ...
    store_path = '{}/multi-pattern-builder2/patterns-{}/{}-{}-{}.txt'.format(file_storage_path,
...
def produce_pattern_group(video_name, params, pattern_num):
  # read meta data.
  meta_data_file = '{}/multi-raw/{}-meta.txt'.format(file_storage_path, video_name)
  # collect 
  intervals = [(z,z+y-1) for x,y,z in  parse_meta_file(meta_data_file)]
...
def build_patterns_from_file(video_name, df_name, obj_num, pattern_length, pattern_num):
  file_store_path  = '{}/multi-pattern-builder2/patterns-{}/{}-{}-{}.txt'.format(file_storage_path,
    video_name, obj_num, pattern_length, pattern_num)
  for idx, frame_pair, ids in read_patterns_from_file(file_store_path):
    print('params', video_name, df_name, frame_pair, ids)
    output_path = '{}/multi-pattern-builder2/patterns-{}/{}-{}-{}/{}-{}-{}-{}.pkl'.format(file_storage_path,
...
def query_with_patterns(pattern_config, query_config):
  ...
  pattern_folder_path = '{}/multi-pattern-builder2/patterns-{}/{}-{}-{}'.format(file_storage_path, 
        video_name, obj_num, pattern_length, pattern_num)
  pattern_txt_file  = '{}/multi-pattern-builder2/patterns-{}/{}-{}-{}.txt'.format(file_storage_path,
    video_name, obj_num, pattern_length, pattern_num)
  ...
  result_folder_path = '{}/multi-batch-results2/patterns-{}/{}-{}-{}-{}'.format(file_storage_path, 
...
def run_query(video_name, pattern_path, idx, frame_pair, ids, df, k, method, result_folder_path):
  index_type = 1
  index_path = '{}/multi-index/{}-{}-{}.pkl'.format(file_storage_path, video_name, index_type, df)
...
def batch_query():
  ...
  for video in videos:
    ...
    default_query_params = [video, 100]
